...
across platforms, because normalization, softmax, rotary embeddings, and
dequantization remain floating point. We conclude that quantization can deliver
exact numerical batch-invariance but not, in this prototype, cross-platform
determinism, and we frame integer-only NanoChat inference as the follow-on work.
\end{abstract}
...
invariance extends beyond one device, we replicate the four core methods on a CPU
platform, where \texttt{\_int\_mm} executes but the surrounding operations use
entirely different kernels. Table~\ref{tab:crossplatform} reports the GPU$\to$CPU
comparison. Preliminary single-condition runs show that static W8A8 logits differ
between GPU and CPU (mean $|\Delta|$ 0.692 on GPU vs.\ 0.695 on CPU), i.e.\
integer accumulation removes \emph{batch}-dependent variation but not
\emph{platform}-dependent variation, because dequantization, normalization,
softmax, and rotary embeddings remain floating point. This is the expected
negative result and directly motivates full integer-only inference.

\begin{table}[t]
\centering
\caption{Cross-platform (GPU vs.\ CPU) replication of the core methods.}
\label{tab:crossplatform}
\footnotesize
\input{tables/crossplatform.tex}
\end{table}
...
invariance does not survive a change of hardware platform because most of the
network is still floating point. On a single modern GPU, greedy inference is
already batch-invariant at the token level, so the value of quantization here is
exact numerical, not behavioral, reproducibility. Full integer-only NanoChat
inference, which could turn this batch-invariance into cross-platform
determinism, is the natural follow-on work.
